...
/*
	...
	mqtt协议简介
		mqtt协议包可以简单的理解为就以下的这两个字段，不管是server-to-client还是client-to-server
	...
	scheme
		...
		4.业务模块
			1）.shceme用于指定mqant后端微服务的模块ID
			2）.hostname用于指定业务模块节点路由规则
				业务模块可能有多个节点
				1.modulus
					取模
				2.cache
			...
			代表后端访问的im模块，按取模的规则选择im模块的一个节点
			访问方法/remove_feeds_member
			msg_id=1002用来唯一标识一次请求
			body:
				{
					"member":"要移除成员userId",
					"feeds":"feedsid"
	...
	服务器响应
		请看，服务器响应消息时topic跟客户端请求时完全相同，因此客户端只要保证topic每一次请求都是唯一
		的，并且记住它，那么在服务器响应时就能找到正确的处理代码了 topic:
			im://modulus/remove_feeds_member?msg_id=1002
		body:
			{
	...
	服务器PUSH模式
		当用户A给用户B发消息时，im工具需要主动的通知用户B消息
		这样情况一般我们回提前跟服务器约定好这类消息的topic
		eg.
			通过feeds有新消息
			...
			body:
				{
					...
					"message":{
						...
						"SeqId":int // 消息顺序ID
						"MsgType":string // 消息类型text image 语音：voice 视频：voice 小视频：shortvideo 地理位置：location 连接消息：link
						*"From":string // 谁发出的
						"Payload":Object // 谁发出的
